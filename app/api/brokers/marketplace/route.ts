...
async function payloadFor(brokerId: string, slug: string) {
  const [profile, settings, properties, leads, conversations, reviewCounts] = await Promise.all([
    ...
    prisma.broker.findUnique({ where: { id: brokerId }, select: { catalogSlug: true, marketplaceSpecialty: true, marketplaceRegion: true, marketplaceTransactions: true, marketplaceAbout: true, user: { select: { name: true, photoUrl: true } } } }),
...
export async function PATCH(request: NextRequest) {
  ...
  const about = typeof body?.about === 'string' ? body.about.trim().slice(0, 900) : ''
  const transactions = ['SALE', 'RENT', 'BOTH'].includes(body?.transactions) ? body.transactions : 'BOTH'
  await prisma.broker.update({ where: { id: user.broker.id }, data: { marketplaceSpecialty: specialty || null, marketplaceRegion: region || null, marketplaceAbout: about || null, marketplaceTransactions: transactions } })
  return NextResponse.json(await payloadFor(user.broker.id, user.broker.catalogSlug))
}
