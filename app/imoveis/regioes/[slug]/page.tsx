...
export default async function RegiaoPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <PageShell>
    ...
    <section className="mx-auto w-full max-w-6xl px-5 py-10 md:px-8"><div className="flex items-end justify-between gap-4"><SectionHeading title={`Imóveis em ${region.name}`} support={`${region.properties} ${region.properties === 1 ? 'imóvel publicado' : 'imóveis publicados'} agora.`} /><Link href={`/imoveis/busca?cidade=${region.slug}`} className="inline-flex shrink-0 items-center gap-2 text-sm font-medium text-primary">Ver todos<ArrowRight className="h-4 w-4" /></Link></div><div className="mt-8 grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">{regionProperties.slice(0, 6).map((property) => <PropertyCard key={property.slug} property={{ slug: property.slug, title: property.title, city: property.city, state: property.state, price: property.price, bedrooms: property.bedrooms, area: property.area, parking: property.parking, image: property.image }} />)}</div></section>
    {regionBrokers.length ? <section className="mx-auto w-full max-w-6xl px-5 py-10 pb-20 md:px-8"><SectionHeading title="Quem atua nesta região" support="Corretores vinculados ao inventário publicado." /><div className="mt-8 grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">{regionBrokers.slice(0, 3).map((broker) => <BrokerCard key={broker.slug} broker={broker} />)}</div></section> : null}
  </PageShell>
}
