    ...
    <div className="grid min-w-0 gap-5">
      <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
        <Card className="min-w-0 overflow-hidden rounded-[1.75rem] border-black/[0.06] bg-white/90 py-0">
            ...
            <div className="flex flex-wrap gap-2 pt-3">
              {filters.map((item) => (
                <button
                  ...
                  className={`rounded-full border px-3 py-1.5 text-sm transition ${filter === item.value ? "border-[#009b3a]/25 bg-[#009b3a]/10 text-[#009b3a]" : "border-black/[0.06] bg-[#fbfbf8] text-[#5F6B7A] hover:bg-[#f6f7f4]"}`}
                >
                  {item.label}
                </button>
              ))}
          ...
          <CardContent className="grid gap-3 p-5 pt-0">
            ...
            {isLoading ? (
              ...
              events.map((event) => (
                <div key={event.id} className="grid gap-3 rounded-[1.25rem] border border-black/[0.06] bg-[#fbfbf8] p-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
                  <div className="min-w-0">
                    <p className="truncate font-semibold text-[#050505]">{event.title}</p>
                    <p className="mt-1 flex flex-wrap items-center gap-2 text-sm text-[#6B7280]">
                      <Clock className="size-4 text-[#009b3a]" />
                      {event.date} {event.time || ""}
                      {event.propertyTitle ? `· ${event.propertyTitle}` : ""}
                      {event.leadName ? `· ${event.leadName}` : ""}
                    </p>
                    {event.notes ? <p className="mt-2 line-clamp-2 text-sm text-[#5F6B7A]">{event.notes}</p> : null}
                  </div>
                  <Button type="button" variant="ghost" disabled={event.status === "done"} onClick={() => markDone(event.id)} className="h-10 rounded-xl border border-black/[0.06] bg-white/80 text-[#4B5563] hover:bg-white">
              ...
              <p className="rounded-[1.25rem] border border-black/[0.06] bg-[#fbfbf8] p-4 text-sm text-[#6B7280]">Você não tem compromissos nessa data.</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="min-w-0 overflow-hidden rounded-[1.75rem] border-black/[0.06] bg-white/90 py-0">
          ...
          <CardContent className="grid gap-3 p-5 pt-0">
            <Input value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} placeholder="Ex: Visita com João" className="h-10 rounded-xl border-black/[0.06] bg-white/80 text-[#050505]" />
            <select value={draft.type} onChange={(event) => setDraft({ ...draft, type: event.target.value })} className="h-10 rounded-xl border border-black/[0.06] bg-white/80 px-3 text-sm text-[#050505]">
              <option value="visit" className="bg-white">Visita</option>
              <option value="reminder" className="bg-white">Lembrete</option>
              <option value="event" className="bg-white">Evento</option>
              <option value="task" className="bg-white">Tarefa</option>
            </select>
            <div className="grid gap-3 sm:grid-cols-2">
              <Input type="date" value={draft.date} onChange={(event) => setDraft({ ...draft, date: event.target.value })} className="h-10 rounded-xl border-black/[0.06] bg-white/80 text-[#050505]" />
              <Input type="time" value={draft.time} onChange={(event) => setDraft({ ...draft, time: event.target.value })} className="h-10 rounded-xl border-black/[0.06] bg-white/80 text-[#050505]" />
            </div>
            <Textarea value={draft.notes} onChange={(event) => setDraft({ ...draft, notes: event.target.value })} placeholder="Observações" className="min-h-24 rounded-xl border-black/[0.06] bg-white/80 text-[#050505]" />
            <Button type="button" disabled={isSaving} onClick={createEvent} className="h-10 rounded-xl bg-[#009b3a] text-sm font-semibold text-white hover:bg-[#008633] disabled:opacity-60">
