  ...
  return <BrokerPageShell title="Catálogo" contentClassName="pb-10">
    <div className="space-y-4 p-3 sm:p-0">
      <BrokerPageIntro
        ...
        title="Gerencie sua vitrine pública"
        description="Identidade, link e uma prévia compacta do catálogo que seus clientes acessam."
      ...
      <div className="grid items-start gap-4 lg:grid-cols-[minmax(17rem,.72fr)_minmax(0,1.28fr)]">
        ...
        <BrokerSurface padding="compact">
          ...
          <div className="mt-4 flex items-center gap-3"><button type="button" onClick={() => photoInput.current?.click()} className="group relative flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-black/[0.08] bg-[#f5f6f3]">{draft.photoUrl ? <Image src={draft.photoUrl} alt="Foto do catálogo" fill sizes="64px" className="object-cover" /> : <ImagePlus className="h-5 w-5 text-[#8b95a1]" />}<span className="absolute inset-x-0 bottom-0 bg-black/55 py-1 text-[10px] text-white opacity-100 transition-opacity sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-visible:opacity-100">Alterar</span></button><div><p className="text-sm font-semibold">Foto do catálogo</p><p className="mt-1 text-xs leading-relaxed text-[#667085]">JPG, PNG ou WebP. A imagem é otimizada antes de salvar.</p><input ref={photoInput} type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={(event) => void uploadPhoto(event.target.files?.[0])} /></div></div>
          <div className="mt-4 space-y-3"><label className="block text-sm font-medium text-[#344054]">Nome público<input required maxLength={120} value={draft.displayName} onChange={(event) => setDraft({ ...draft, displayName: event.target.value })} className="mt-1.5 h-10 w-full rounded-lg border border-black/[0.09] bg-white px-3 text-[#050505] outline-none transition focus:border-[#009b3a]/45 focus:ring-2 focus:ring-[#009b3a]/8" /></label><label className="block text-sm font-medium text-[#344054]">Endereço do catálogo<div className="mt-1.5 flex h-10 items-center overflow-hidden rounded-lg border border-black/[0.09] bg-white focus-within:border-[#009b3a]/45 focus-within:ring-2 focus-within:ring-[#009b3a]/8"><span className="border-r border-black/[0.06] bg-[#f7f8f5] px-3 text-xs text-[#667085]">/catalogo/</span><input required value={draft.slug} onChange={(event) => setDraft({ ...draft, slug: event.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '-') })} className="min-w-0 flex-1 px-3 text-sm outline-none" /></div></label><label className="block text-sm font-medium text-[#344054]">Descrição<textarea maxLength={600} rows={4} value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} placeholder="Apresente seu atendimento e a seleção de imóveis do catálogo." className="mt-1.5 w-full resize-none rounded-lg border border-black/[0.09] px-3 py-2.5 text-sm outline-none transition focus:border-[#009b3a]/45 focus:ring-2 focus:ring-[#009b3a]/8" /></label></div>
          <div className="mt-4 flex flex-wrap items-center gap-3"><button disabled={saving} className="inline-flex h-9 items-center gap-2 rounded-lg bg-[#009b3a] px-4 text-xs font-semibold text-white shadow-sm shadow-[#009b3a]/12 disabled:opacity-60"><Save className="h-4 w-4" />{saving ? 'Salvando...' : 'Salvar catálogo'}</button>{feedback ? <p role="status" className="text-sm text-[#667085]">{feedback}</p> : null}</div>
        </BrokerSurface>
        </form>
