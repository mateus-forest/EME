              ...
              <div className="bg-[#f3f5f1] p-4">
                <div className="overflow-hidden rounded-2xl border border-black/[0.07] bg-white shadow-[0_18px_45px_rgba(15,23,42,.08)]">
                    ...
                    <img src={draft.bannerUrl || '/marketplace/images/hero-residence.png'} alt="" className="absolute inset-0 h-full w-full object-cover" />
                      ...
                      <div className="relative flex min-h-52 items-center gap-4 px-5 pb-4 pt-7">
                      <div className="relative size-20 shrink-0 overflow-hidden rounded-full border-2 border-white bg-[#eef1ec] shadow-md">{draft.photoUrl ? <Image src={draft.photoUrl} alt="" fill sizes="80px" className="object-cover" /> : null}</div>
                        <div className="min-w-0 text-center"><p className="truncate text-lg font-semibold text-[#08110b] [text-shadow:0_1px_14px_rgba(255,255,255,.72)]">{draft.displayName || 'Seu nome'}</p><BrokerSpecialtyChips specialties={draft.specialties} compact emptyLabel="Suas especialidades aparecerão aqui." className="mt-2 justify-center" />{draft.creci ? <p className="mt-2 inline-flex items-center gap-1.5 rounded-full border border-white/80 bg-white/78 px-2.5 py-1 text-[11px] font-semibold text-[#294536] shadow-sm">{draft.creciVerified ? <BadgeCheck className="size-3.5 fill-[#17a24c] text-white" /> : null}CRECI {draft.creci}{draft.creciUf ? ` · ${draft.creciUf}` : ''}</p> : null}</div>
                    </div>
                  </div>
