...
export function BrokerRentalStartDialog({ property, open, onOpenChange, onCreated }: { property: RentalProperty | null; open: boolean; onOpenChange: (open: boolean) => void; onCreated: () => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[92vh] max-w-[calc(100%-1.5rem)] overflow-x-hidden overflow-y-auto rounded-[1.5rem] border-black/[0.06] bg-white p-0 sm:max-w-3xl">
      ...
      <div className="grid min-w-0 gap-4 px-4 py-5 sm:grid-cols-2 sm:px-6">
        ...
        <div className="min-w-0 sm:col-span-2"><RentalField label="Contrato"><Select value={form.contractDocumentId} onValueChange={(value) => setForm({ ...form, contractDocumentId: value })}><SelectTrigger className="w-full min-w-0"><SelectValue placeholder="Selecione um contrato" /></SelectTrigger><SelectContent>{contracts.map((contract) => <SelectItem key={contract.id} value={contract.id}>{contract.title}</SelectItem>)}</SelectContent></Select><Input className="mt-2" type="file" accept=".pdf,.doc,.docx" onChange={(event) => setContractFile(event.target.files?.[0] ?? null)} /><Link href="/corretor/documentos/contratos" className="mt-1 inline-flex text-xs font-medium text-[#008633]">Criar ou relacionar em Contratos</Link></RentalField></div>
...
function RentalField({ label, children }: { label: string; children: React.ReactNode }) { return <label className="grid min-w-0 gap-1.5 text-sm font-medium text-[#344054]"><span>{label}</span>{children}</label> }
