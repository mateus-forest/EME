...
import { ArrowLeft, Check, Library, LoaderCircle, Megaphone, Sparkles } from "lucide-react"
    ...
    <BrokerPageShell title="Studio IA">
      <div className="grid min-w-0 gap-4 sm:gap-5">
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          <Card className="rounded-[1.5rem] border-black/[0.06] py-0"><CardHeader className="px-5 py-5"><CardTitle>1. Material</CardTitle></CardHeader><CardContent className="grid gap-4 px-5 pb-6">{isLoading ? <p className="flex items-center gap-2 text-sm text-[#667085]"><LoaderCircle className="size-4 animate-spin" />Carregando Biblioteca...</p> : materials.length ? <Select value={sourceAssetId} onValueChange={setSourceAssetId}><SelectTrigger data-testid="ad-library-material"><SelectValue placeholder="Escolha um material" /></SelectTrigger><SelectContent>{materials.map((item) => <SelectItem key={item.asset.id} value={item.asset.id}>{item.campaign.title} · {item.asset.label || item.asset.type}</SelectItem>)}</SelectContent></Select> : <div className="rounded-xl border border-dashed border-black/[0.08] bg-[#fbfbf8] p-5 text-sm text-[#667085]">Aprove uma imagem ou vídeo na Biblioteca para criar um anúncio.</div>}{selected?.asset.fileUrl ? selected.asset.type === "VIDEO" ? <video src={selected.asset.fileUrl} controls className="max-h-80 w-full rounded-2xl bg-black" /> : <img src={selected.asset.fileUrl} alt="Material selecionado" className="max-h-80 w-full rounded-2xl object-contain bg-[#f5f6f4]" /> : null}</CardContent></Card>
          <Card className="rounded-[1.5rem] border-black/[0.06] py-0"><CardHeader className="px-5 py-5"><CardTitle>2. Direção do anúncio</CardTitle></CardHeader><CardContent className="grid gap-4 px-5 pb-6"><label className="grid gap-2 text-sm font-medium">Onde pretende anunciar?<Select value={channel} onValueChange={(value) => setChannel(value as typeof channel)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{channels.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select></label><label className="grid gap-2 text-sm font-medium">Objetivo<Select value={objective} onValueChange={(value) => setObjective(value as typeof objective)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{objectives.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select></label><div className="rounded-xl border border-black/[0.06] bg-[#fbfbf8] p-4 text-xs leading-5 text-[#667085]">A OpenAI gera somente o conteúdo comercial. O material selecionado permanece vinculado ao anúncio.</div><Button onClick={generate} disabled={!sourceAssetId || isGenerating} className="h-11 rounded-xl">{isGenerating ? <LoaderCircle className="size-4 animate-spin" /> : <Sparkles className="size-4" />}{isGenerating ? "Gerando anúncio..." : "Gerar anúncio"}</Button></CardContent></Card>
        </section>
        {result ? <Card className="rounded-[1.5rem] border-black/[0.06] py-0"><CardHeader className="px-5 py-5"><CardTitle>Resultado utilizável</CardTitle></CardHeader><CardContent className="grid gap-3 px-5 pb-6"><ResultBlock label="Título" value={result.title} /><ResultBlock label="Texto principal" value={result.primaryText} /><ResultBlock label="CTA" value={result.cta} /><ResultBlock label="Público sugerido" value={result.audience} /><ResultBlock label="Abordagem" value={result.approach} /><div className="mt-2 flex flex-wrap gap-2">{campaign?.status === "APPROVED" ? <Button disabled><Check className="size-4" />Aprovado</Button> : <Button onClick={approve} disabled={isApproving}>{isApproving ? <LoaderCircle className="size-4 animate-spin" /> : <Check className="size-4" />}Aprovar anúncio</Button>}<Button asChild variant="outline"><Link href="/corretor/studio-ia/biblioteca"><Library className="size-4" />Abrir Biblioteca</Link></Button></div></CardContent></Card> : null}
      </div>
    </BrokerPageShell>
  )
}
