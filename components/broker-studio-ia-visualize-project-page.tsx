...
import { ArrowLeft, Check, Library, LoaderCircle, Megaphone, Sparkles, Upload, Video } from "lucide-react"
    ...
    <BrokerPageShell title="Studio IA">
      <div className="grid min-w-0 gap-4 sm:gap-5">
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          <Card className="rounded-[1.5rem] border-black/[0.06] bg-white/90 py-0"><CardHeader className="px-5 py-5"><CardTitle>1. Imagem e objetivo</CardTitle></CardHeader><CardContent className="grid gap-4 px-5 pb-6">
            <label className="cursor-pointer rounded-2xl border border-dashed border-black/[0.09] bg-[#fbfbf8] p-4"><div className="flex items-center gap-3"><Upload className="size-5 text-[#009b3a]" /><div><p className="text-sm font-semibold">Enviar imagem</p><p className="text-xs text-[#667085]">Terreno, obra, render, projeto ou fachada · até 15 MB</p></div></div><Input className="hidden" type="file" accept="image/png,image/jpeg,image/webp" onChange={handleImage} /></label>
          ...
          <Card className="rounded-[1.5rem] border-black/[0.06] bg-white/90 py-0"><CardHeader className="px-5 py-5"><CardTitle>2. IA e alternativas</CardTitle></CardHeader><CardContent className="grid gap-4 px-5 pb-6">
