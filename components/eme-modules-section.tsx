  ...
  return (
    <section id="modulos-eme" className="relative z-10 px-4 py-24 md:py-32">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {modules.map((module) => (
            <button
              ...
              className="group min-h-[13rem] rounded-[1.5rem] border border-white/[0.08] bg-white/[0.03] p-5 text-left transition-all duration-300 hover:border-[#00C853]/30 hover:bg-[#00C853]/[0.045] hover:shadow-[0_0_42px_rgba(0,200,83,0.12)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00C853]/70"
            >
              <span className="flex size-12 items-center justify-center rounded-2xl border border-[#00C853]/20 bg-[#00C853]/10 text-[#69F0AE] transition-colors duration-300 group-hover:border-[#00C853]/35 group-hover:bg-[#00C853]/15">
                <module.icon className="size-5" />
              </span>
              <span className="mt-7 block text-xl font-semibold text-white">{module.title}</span>
              <span className="mt-3 block text-sm leading-6 text-white/52">{module.text}</span>
              <span className="mt-6 inline-flex items-center gap-2 text-sm font-medium text-[#69F0AE]/90">
                Explorar módulo
      ...
      <Dialog open={Boolean(selectedModule)} onOpenChange={(open) => !open && setSelectedModule(null)}>
        <DialogContent className="max-h-[calc(100vh-2rem)] overflow-y-auto rounded-[1.75rem] border-white/[0.08] bg-[#0B0F0D] p-0 text-white shadow-[0_32px_100px_rgba(0,0,0,0.65),0_0_80px_rgba(0,200,83,0.14)] sm:max-w-xl">
          {selectedModule ? (
            <div className="relative overflow-hidden rounded-[1.75rem]">
              <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_0%,rgba(0,200,83,0.18),transparent_38%),linear-gradient(180deg,rgba(255,255,255,0.05),transparent)]" />
              <div className="relative p-6 sm:p-8">
                <div className="flex size-12 items-center justify-center rounded-2xl border border-[#00C853]/22 bg-[#00C853]/12 text-[#69F0AE]">
                  <selectedModule.icon className="size-5" />
                </div>
                <DialogTitle className="mt-6 text-2xl font-bold leading-tight text-white">
                  {selectedModule.modalTitle}
                </DialogTitle>
                <DialogDescription className="mt-3 text-base leading-7 text-white/60">
                ...
                <div className="mt-7 grid gap-3 sm:grid-cols-2">
                  {selectedModule.bullets.map((bullet) => (
                    <div
                      key={bullet}
                      className="flex items-center gap-3 rounded-2xl border border-white/[0.08] bg-white/[0.035] px-4 py-3 text-sm font-medium text-white/82"
                    >
                      <CheckCircle2 className="size-4 shrink-0 text-[#69F0AE]" />
                      <span>{bullet}</span>
                    </div>
                  ))}
                </div>
                ...
                <Link
                  ...
                  className="mt-8 inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-[#00C853] px-6 text-sm font-bold text-black shadow-[0_0_30px_rgba(0,200,83,0.24)] transition-all duration-300 hover:bg-[#00E676] hover:shadow-[0_0_40px_rgba(0,200,83,0.36)]"
                >
                  Explorar módulo
