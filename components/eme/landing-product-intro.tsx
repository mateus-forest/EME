  ...
  return <>
    ...
    {cosOpen && <LandingModalShell label="COS, o assistente do EME" moduleId="cos-info" presentation originEl={origin.current} onClose={() => setCosOpen(false)}>
      <div className={styles.cosContent}>
        <MessageCircle data-choreography="0" size={30} aria-hidden />
        <h2 data-choreography="1">COS, o assistente do EME.</h2>
        <p data-choreography="2">Consulte informações e acesse ações da sua operação por conversa, com passos claros para revisar e concluir.</p>
        <ul data-choreography="3"><li>Consulte clientes, imóveis, agenda e o resumo financeiro.</li><li>Acesse formulários de ações disponíveis, como cadastrar um cliente ou registrar um compromisso.</li><li>Revise e confirme os dados solicitados para concluir cada ação.</li></ul>
        <p data-choreography="4" className={styles.note}>As ações são delimitadas e podem utilizar Créditos IA. O COS não é um agente autônomo nem um monitor contínuo. Uma experiência mais ampla está em desenvolvimento.</p>
      </div>
    </LandingModalShell>}
  </>
}
