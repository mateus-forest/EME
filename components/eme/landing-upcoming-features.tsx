...
export function LandingUpcomingFeatures({ className, compact = false }: { className?: string; compact?: boolean }) {
  return (
    ...
    >
      ...
      <div className={compact ? "flex gap-[2px]" : "flex gap-1.5"}>
        {upcomingFeatures.map((lines) => (
          <article
            key={lines[0]}
            className={
              compact
                ? "flex h-[44px] w-[38px] flex-col items-center rounded-[8px] border border-white/45 bg-white/40 px-[2px] pb-1 pt-1 text-center shadow-[0_8px_20px_-16px_rgba(35,43,47,0.24)] backdrop-blur-md"
          ...
          >
            ...
            <LoaderCircle
              aria-hidden="true"
              className={`${compact ? "mt-0.5 size-2" : "mt-3 size-4"} animate-spin text-[#ffb000] [animation-duration:1.8s]`}
              strokeWidth={1.7}
            />
            ...
            <p
              className={
                compact
                  ? "mt-auto text-[5.5px] font-semibold leading-none tracking-[-0.03em] text-[#171d20]"
