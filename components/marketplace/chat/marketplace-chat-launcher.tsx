...
type MarketplaceChatLauncherProps = {
  brokerSlug: string
  brokerName: string
  brokerPhone?: string
  propertyId?: string
...
export function MarketplaceChatLauncher({
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => void loadConversation(conversation.token).catch(() => null), 5_000)
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (conversation) {
        const response = await fetch(`/api/marketplace/conversations/${conversation.token}`, {
          method: 'POST',
        ...
        const response = await fetch('/api/marketplace/conversations', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ brokerSlug, propertyId, customerName: name, customerPhone: normalizePhone(phone), message }),
        })
        const payload = await response.json()
    ...
    {open ? <div className="fixed inset-0 z-[120] flex items-end justify-center sm:items-center sm:p-6">
      ...
      <section role="dialog" aria-modal="true" aria-label={`Conversa com ${brokerName}`} className="relative flex h-[min(92dvh,700px)] w-full flex-col overflow-hidden rounded-t-[1.75rem] border border-border bg-background shadow-[var(--shadow-float)] sm:max-w-lg sm:rounded-[1.75rem]">
        ...
        <div className="flex-1 space-y-3 overflow-y-auto bg-secondary/30 px-5 py-5">
          {!conversation ? <div className="rounded-2xl border border-border bg-card p-4 text-sm leading-relaxed text-muted-foreground">Inicie uma conversa interna. O corretor será notificado e o histórico continuará disponível no EME.</div> : conversation.messages.map((item) => <div key={item.id} className={cn('flex', item.sender === 'CUSTOMER' ? 'justify-end' : 'justify-start')}>{item.kind && item.kind !== 'TEXT' ? <MarketplaceMessageCard kind={item.kind} body={item.body} metadata={item.metadata} /> : <div className={cn('max-w-[84%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed', item.sender === 'CUSTOMER' ? 'rounded-br-md bg-primary text-primary-foreground' : 'rounded-bl-md border border-border bg-card text-foreground')}>{item.body}</div>}</div>)}
...
function ReviewForm({ token, existing, requested, onSaved }: { token: string; existing: Conversation['review']; requested: boolean; onSaved: () => void }) {
  ...
  return <form onSubmit={async (event) => {
    ...
    else setError((await response.json()).error || 'Não foi possível avaliar.')
  }} className="rounded-2xl border border-border bg-card p-4">
    <p className="font-medium text-foreground">{requested ? 'Como foi o atendimento?' : 'Atendimento encerrado'}</p>
    <p className="mt-1 text-xs text-muted-foreground">Sua avaliação entra como pendente e só aparece após aprovação.</p>
    <div className="mt-3 flex gap-1">{[1, 2, 3, 4, 5].map((value) => <button key={value} type="button" onClick={() => setRating(value)} aria-label={`${value} estrelas`}><Star className={cn('h-6 w-6', value <= rating ? 'fill-amber-400 text-amber-400' : 'text-muted-foreground/30')} /></button>)}</div>
    <textarea required minLength={3} rows={3} value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Conte como foi o atendimento" className="mt-3 w-full resize-none rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary/40" />
    <button className="mt-3 rounded-full bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground">Enviar avaliação</button>
