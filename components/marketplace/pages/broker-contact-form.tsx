  ...
  return (
    <form onSubmit={submit} className="rounded-[1.5rem] border border-border/70 bg-card p-6 shadow-[var(--shadow-soft)]">
      ...
      <div className="mt-5 flex flex-col gap-4">
        ...
        <label className="flex flex-col gap-1.5 text-sm font-medium text-foreground">O que você procura<textarea value={message} onChange={(event) => setMessage(event.target.value)} rows={3} placeholder="Ex.: casa com pátio até R$ 750 mil" className="resize-none rounded-xl border border-border bg-background px-3 py-2.5 font-normal text-foreground outline-none focus:border-primary/25 focus:ring-2 focus:ring-primary/5" /></label>
      </div>
