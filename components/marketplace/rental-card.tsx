...
export function RentalCard({
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[1.75rem] border border-border/70 bg-card shadow-[var(--shadow-soft)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[var(--shadow-float)]">
      ...
      >
        <Link href={`/imoveis/imovel/${rental.slug}`} className="absolute inset-0">
          <Image
            src={rental.image || '/marketplace/placeholder.svg'}
            alt={rental.title}
            fill
            ...
            <div className="shrink-0 text-right">
              ...
              {rental.condo ? (
                <p className="mt-0.5 text-xs text-muted-foreground">
                  Cond. {formatMonthly(rental.condo)}
                </p>
              ) : (
                <p className="mt-0.5 text-xs text-muted-foreground">Sem condomínio</p>
              )}
            </div>
          </div>
        ...
        <div className="mt-5 flex items-center gap-4 border-t border-border/60 pt-4 text-sm text-muted-foreground">
          {rental.commercial ? (
            ...
            <span className="inline-flex items-center gap-1.5">
              ...
              {rental.bedrooms} {rental.bedrooms > 1 ? 'quartos' : 'quarto'}
            </span>
          )}
          <span className="inline-flex items-center gap-1.5">
          ...
          <span className="inline-flex items-center gap-1.5">
            ...
            {rental.parking} {rental.parking > 1 ? 'vagas' : 'vaga'}
          </span>
        </div>
      </div>
    </article>
