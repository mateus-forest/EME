...
export function Popover({ label, active = false, align = 'start', children, className }: { label: React.ReactNode; active?: boolean; align?: 'start' | 'end'; children: (close: () => void) => React.ReactNode; className?: string }) {
  ...
  return <div className="relative">
    ...
    {open && typeof document !== 'undefined' ? createPortal(<div ref={panelRef} role="dialog" style={{ position: 'fixed', top: position.top, left: position.left, width: position.width }} className={cn('z-[150] max-h-[min(70dvh,28rem)] overflow-y-auto rounded-2xl border border-border/70 bg-popover p-4 shadow-[var(--shadow-float)] motion-safe:animate-in motion-safe:fade-in-0 motion-safe:zoom-in-95 motion-safe:duration-150', className)}>{children(() => setOpen(false))}</div>, document.body) : null}
  </div>
}
