        ...
        {regions.slice(0, 3).map((region, index) => (
          <Reveal key={region.slug} delay={index * 90}>
            <Link href={`/imoveis/regioes/${region.slug}`} className="group relative block aspect-[4/3] overflow-hidden rounded-[1.75rem] shadow-[var(--shadow-soft)] transition-all duration-500 hover:-translate-y-1 hover:shadow-[var(--shadow-float)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring md:aspect-[4/5] lg:aspect-[4/3]">
              {region.image ? <Image src={region.image} alt="" fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover transition-transform duration-700 ease-out group-hover:scale-105" /> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_75%_20%,rgba(35,120,55,.22),transparent_38%),linear-gradient(145deg,#1f2d25,#102018)] transition-transform duration-700 ease-out group-hover:scale-105" />}
              {region.image ? <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/15 to-black/5" /> : null}
