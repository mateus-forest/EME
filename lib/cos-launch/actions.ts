...
const capabilityByForm: Record<CosLaunchFormKind, CosCapabilityId> = { client: "lead.create", property: "property.create", proposal: "proposal.create", contract: "contract.create", agenda: "agenda.create", document: "lead.attach_document" }
const value = (payload: Record<string, unknown>, key: string) => typeof payload[key] === "string" ? (payload[key] as string).trim() : ""
function actionMessage(kind: CosLaunchFormKind, payload: Record<string, unknown>) { if (kind === "client") return `Cadastre o cliente ${value(payload, "name")} com WhatsApp ${value(payload, "phone")}.`; if (kind === "property") return `Crie um rascunho do imóvel ${value(payload, "title")} por ${value(payload, "price")}.`; if (kind === "proposal") return `Crie uma proposta para o cliente e imóvel selecionados no valor de ${value(payload, "value")}.`; if (kind === "contract") return `Crie um rascunho de contrato do tipo ${value(payload, "contractType") || "Contrato imobiliário"}.`; if (kind === "agenda") return `Crie o compromisso ${value(payload, "title")} em ${value(payload, "date")} às ${value(payload, "time")}.`; return `Anexe o documento ${value(payload, "fileName") || "selecionado"} ao cliente selecionado.` }
...
export async function executeCosLaunchAction(input: { kind: CosLaunchFormKind; brokerId: string; userId: string; payload: Record<string, unknown> }) {
  const capabilityId = capabilityByForm[input.kind]; const descriptor = getCosCapabilityDescriptorById(capabilityId); const entity = getCosEntityModuleIdByCapabilityId(capabilityId); const handler = capabilityHandlers[capabilityId]
  ...
  const raw = await handler({ brokerId: input.brokerId, userId: input.userId, message: actionMessage(input.kind, input.payload), action: descriptor.action, confirm: true, payload: input.payload, pendingInput: null, context: null })
