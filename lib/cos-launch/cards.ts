...
function source(value: string | null) { const normalized = value?.toLowerCase() ?? ""; if (normalized.includes("marketplace")) return "Marketplace"; if (normalized.includes("catalog")) return "Catálogo"; if (normalized.includes("cos")) return "COS"; return "Manual" }
