...
const frontOrder = [
  ["imoveis", "Imóveis"], ["catalogo", "Catálogo e marketplace"], ["studio-ia", "Studio IA"],
  ["propostas", "Propostas e contratos"], ["financeiro", "Financeiro"], ["clientes", "Clientes e agenda"],
] as const
...
async function openAtPoint(page: Page, id: string, button: Locator, touch = false) {
  ...
  await expect(dialog).toHaveCSS("opacity", /^(1|0\.99\d*)$/)
  await expect(dialog.locator("[data-eme-modal-content]")).toHaveCSS("scrollbar-width", "none")
  if (!touch) {
...
test("front cards receive real hover/click throughout a full desktop rotation", async ({ page }) => {
  ...
  await expect(page.locator('[data-orbit-card="imoveis"]')).toHaveCSS("opacity", /^(1|0\.99\d*)$/)
  await page.waitForTimeout(900)
  await expect(page.locator("[data-orbit-card]")).toHaveCount(6)
  ...
  for (const [id, name] of frontOrder) {
    ...
    await expect(page.locator("[data-orbit-cursor] > div")).toHaveCSS("opacity", /^(1|0\.99\d*)$/)
...
test("reduced motion keeps a visible native pointer on front cards", async ({ page }) => {
  ...
  await page.goto("/")
  const button = page.getByRole("button", { name: "Abrir modulo Imóveis", exact: true })
  await expect(page.locator('[data-orbit-card="imoveis"]')).toHaveCSS("opacity", /^(1|0\.99\d*)$/)
  await page.waitForTimeout(400)
  await expect(button).toHaveCSS("cursor", "pointer")
  await expect(page.locator("[data-orbit-cursor]")).toBeHidden()
  await openAtPoint(page, "imoveis", button)
})
...
test("all mobile front cards accept taps after dragging the orbit", async ({ browser, baseURL }) => {
  ...
  try {
    await page.goto("/")
    await expect(page.locator('[data-mobile-orbit-card="imoveis"]')).toHaveCSS("opacity", /^(1|0\.99\d*)$/)
    await page.waitForTimeout(500)
    for (const [id, name] of frontOrder) {
      ...
      for (let step = 0; step < 14; step++) {
        const delta = await card.evaluate(element => {
          ...
          const sin = (matrix.m41 + (element as HTMLElement).offsetWidth / 2) / Math.min(195, (window.innerWidth - 152) / 2)
...
test("depth controls the whole card and hit testing in both scroll directions", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 })
  await page.emulateMedia({ reducedMotion: "reduce" })
  await page.goto("/")
  await expect(page.locator("[data-orbit-card]")).toHaveCount(6)
    ...
    for (let step = 0; step < 5; step++) {
      ...
      const frames = await page.evaluate(async () => {
        ...
        for (let frame = 0; frame < 8; frame++) {
          ...
          document.querySelectorAll<HTMLElement>("[data-orbit-card]").forEach(card => {
            const depth = Number(card.dataset.depth)
            const eased = depth * depth * (3 - 2 * depth)
            const expected = .16 + .84 * eased
            if (Math.abs(Number(card.style.opacity) - expected) > .001) errors.push("Opacity is detached from depth")
            const expectedBrightness = .78 + .22 * eased
            const brightness = Number(getComputedStyle(card).filter.match(/brightness\(([^)]+)\)/)?.[1])
            if (Math.abs(brightness - expectedBrightness) > .001) errors.push("Brightness is detached from depth")
            if (depth < .4 && getComputedStyle(card.querySelector("button")!).pointerEvents !== "none") errors.push("Invisible card intercepts pointers")
            if (getComputedStyle(card).pointerEvents !== "none") errors.push("Structural overlay intercepts pointers")
          })
        }
        return errors
      })
